...
import { _HttpClient, CNCurrencyPipe, YNPipe, ModalHelper, AlainThemeModule } from '@delon/theme';
import { AlainACLModule, ACLService } from '@delon/acl';

import { SimpleTableFilter, SimpleTableColumn, ResReNameType } from './interface';
...
describe('abc: simple-table', () => {
    ...
    function genModule(other: { acl: boolean }) {
        const imports = [
            ...
            AlainThemeModule.forRoot(), AdSimpleTableModule.forRoot()
        ];
    ...
    describe('[property]', () => {
        ...
        xdescribe('#data', () => {
            ...
            describe('with url', () => {
                ...
                const URLTOTAL = 3;
                const URLDATA = {
                    list: Array(PS).fill({}).map((item: any, idx: number) => {
                        return {
                            id: idx + 1,
                            name: `name ${idx + 1}`,
                            age: Math.ceil(Math.random() * 10) + 20,
        ...
        xdescribe('#showTotal', () => {
            it('with false', () => {
                ...
                fixture.detectChanges();
                expect((page.getEl('.ant-pagination-total-text') as HTMLElement).textContent.trim().length).toBe(0);
            });
            it('with true', () => {
                context.showTotal = true;
                fixture.detectChanges();
                expect((page.getEl('.ant-pagination-total-text') as HTMLElement).textContent).toContain(
                    comp.total
                );
            });
            it('should be custom template', () => {
                ...
                fixture.detectChanges();
                expect((page.getEl('.ant-pagination-total-text') as HTMLElement).textContent.trim()).toBe(
    ...
    class PageObject {
        ...
        expectEl(cls: string, result = true): this {
            const el = dl.query(By.css(cls));
            if (result)
                expect(this.getEl(cls)).not.toBeNull();
            else
                expect(this.getEl(cls)).toBeNull();
            return this;
        }
