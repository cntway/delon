import { Component, Inject, Input, Output, OnDestroy, OnInit, OnChanges, SimpleChanges, EventEmitter, Renderer2, ElementRef, TemplateRef, SimpleChange, QueryList, ViewChildren, AfterViewInit, ContentChildren, ContentChild, Optional, ChangeDetectionStrategy, ChangeDetectorRef } from '@angular/core';
...
export class SimpleTableComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
    ...
    @ContentChild('expand') expand: TemplateRef<{ $implicit: any, column: SimpleTableColumn }>;
    ...
    private getAjaxData(url?: string): Observable<any> {
        ...
        return this.http.request(this.reqMehtod || 'GET', url || this._url, {
            ...
            // total
            const retTotal = this._resRN.total && deepGet(res, this._resRN.total as string[], 0);
            this.total = retTotal == null ? 0 : +retTotal;
            return <any[]>ret;
        }));
    }
