...
import { DC_OPTIONS_TOKEN, DEFAULT, CacheOptions } from '../cache.options';
...
describe('cache: service', () => {
    ...
    function genModule(options?: CacheOptions) {
    ...
    describe('[module]', () => {
        it('should be config reName', () => {
            genModule({ reName: 'a' });
            expect(injector.get(DC_OPTIONS_TOKEN).reName).toContain('a');
        });
        it('should be config array reName', () => {
            genModule({ reName: [ 'a', 'b' ] });
            expect(injector.get(DC_OPTIONS_TOKEN).reName).toContain('b');
        });
    });
});
